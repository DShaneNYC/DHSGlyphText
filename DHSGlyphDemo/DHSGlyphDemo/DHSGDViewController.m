...
- (void)setupButton {
	...
	// System Font
    [_glyphButton setSystemFont:[UIFont fontWithName:DHSGLYPHDEMO_SYSTEM_FONT_NAME
                                                size:[_glyphButton glyphLabelForState:UIControlStateNormal].currentFont.fontSize]];
...
- (void)setupLabel {
    ...
    _glyphLabel = (DHSGlyphLabel *)[[DHSGlyphLabelRandomShift alloc] initWithFrame:_glyphLabel.frame];
    ...
    // This can be better handled by making a subclass of DHSGlyphLabelRandomShift and pinning the values
	...
	// System Font
    [_glyphLabel setSystemFont:[UIFont fontWithName:DHSGLYPHDEMO_SYSTEM_FONT_NAME
...
- (void)viewDidLoad {
    ...
    [_titleLabel setText:[NSString stringWithFormat:@"DHSGlyph %@", DHSGlyphTextVersion]];
    ...
    _choiceButtonLabelControl.selectedSegmentIndex = DHSGlyphDemoButtonLableControlTypeButton;
    [self choiceButtonLabelControlTap:nil];
    _choiceGradientFillControl.selectedSegmentIndex = DHSGlyphDemoGradientFillControlTypeNone;
    [self choiceGradientFillControlTap:nil];
    _choiceComponentControl.selectedSegmentIndex = DHSGlyphDemoComponentControlTypeFill;
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    if ([self isCurrentChoiceButton]) {
        // Button
        [_glyphButton setTitle:[_displayTextField.text stringByReplacingCharactersInRange:range withString:string]];
        [_glyphButton setNeedsDisplay];
    } else {
        // Label
        [_glyphLabel setText:[_displayTextField.text stringByReplacingCharactersInRange:range withString:string]];
...
- (BOOL)isCurrentChoiceButton {
    return [_choiceButtonLabelControl selectedSegmentIndex] == DHSGlyphDemoButtonLableControlTypeButton;
}
...
- (IBAction)choiceButtonLabelControlTap:(id)sender {
    switch (_choiceButtonLabelControl.selectedSegmentIndex) {
        case DHSGlyphDemoButtonLableControlTypeButton:
            _choiceRenderOrderControl.selectedSegmentIndex = [_glyphButton glyphLabelForState:UIControlStateNormal].glyphRenderOrder;
            _displayTextField.text = _glyphButton.titleLabel.text;
            _displayTextField.placeholder = @"Button Text";
            _fontSizeStepper.value = [_glyphButton glyphLabelForState:UIControlStateNormal].currentFont.fontSize;
            _fontStrokeWidthStepper.value = [_glyphButton glyphLabelForState:UIControlStateNormal].strokeWidth;
            _strokeEffectSwitch.on = [_glyphButton glyphLabelForState:UIControlStateNormal].strokeHasShadow;
            _fontShadowOffsetStepper.value = [_glyphButton glyphLabelForState:UIControlStateNormal].shadowOffset.width;
            break;
            
        case DHSGlyphDemoButtonLableControlTypeLabel:
            _choiceRenderOrderControl.selectedSegmentIndex = _glyphLabel.glyphRenderOrder;
...
- (IBAction)fontSizeStepperTap:(id)sender {
    if ([self isCurrentChoiceButton]) {
        // Button
        _fontSizeAmtLabel.text = [NSString stringWithFormat:@"%.00f", _fontSizeStepper.value];
...
- (IBAction)fontStrokeWidthStepperTap:(id)sender {
    if ([self isCurrentChoiceButton]) {
        // Button
        _fontStrokeWidthAmtLabel.text = [NSString stringWithFormat:@"%.01f", _fontStrokeWidthStepper.value];
...
- (IBAction)fontShadowOffsetStepperTap:(id)sender {
    if ([self isCurrentChoiceButton]) {
        // Button
        _fontShadowOffsetAmtLabel.text = [NSString stringWithFormat:@"%.00f, %.00f", _fontShadowOffsetStepper.value, _fontShadowOffsetStepper.value];
        [_glyphButton setShadowOffset:CGSizeMake(_fontShadowOffsetStepper.value, _fontShadowOffsetStepper.value)];
        [_glyphButton setNeedsDisplay];
    } else {
        // Label
        _fontShadowOffsetAmtLabel.text = [NSString stringWithFormat:@"%.00f, %.00f", _fontShadowOffsetStepper.value, _fontShadowOffsetStepper.value];
        [_glyphLabel setShadowOffset:CGSizeMake(_fontShadowOffsetStepper.value, _fontShadowOffsetStepper.value)];
        [_glyphLabel setNeedsDisplay];
    }
}
...
- (IBAction)choiceGradientFillControlTap:(id)sender {
	...
	CGFloat components[15] = {
		255.0 / 255.0, 215.0 / 255.0, 23.0 / 255.0, 1.0,    // Light Yellow (opaque)
		190.0 / 255.0, 190.0 / 255.0, 190.0 / 255.0, 1.0,	// Dark Gray (opaque)
		180.0 / 255.0, 140.0 / 255.0, 2.0 / 255.0, 1.0 };	// Dark Yellow (opaque)
...
- (IBAction)choiceComponentControlTap:(id)sender {
    ...
    if ([self isCurrentChoiceButton]) {
        ...
        switch (_choiceComponentControl.selectedSegmentIndex) {
            case DHSGlyphDemoComponentControlTypeFill:
                _eitherExpansionBlurStepper.value = [_glyphButton glyphLabelForState:UIControlStateNormal].currentFont.glyphExpansionMultiplier;
                _eitherExpansionBlurAmtLabel.text = [NSString stringWithFormat:@"%.01f", _eitherExpansionBlurStepper.value];
                
                _strokeEffectSwitch.on = [_glyphButton glyphLabelForState:UIControlStateNormal].showIndividualGlyphStroke;
                break;

            case DHSGlyphDemoComponentControlTypeStroke:
                _eitherExpansionBlurStepper.value = [_glyphButton glyphLabelForState:UIControlStateNormal].currentFont.scaleFactor.x;
                _eitherExpansionBlurAmtLabel.text = [NSString stringWithFormat:@"%.01f", _eitherExpansionBlurStepper.value];
            ...
            case DHSGlyphDemoComponentControlTypeGlow:
                _eitherExpansionBlurStepper.value = [_glyphButton glyphLabelForState:UIControlStateNormal].glowBlur;
                _eitherExpansionBlurAmtLabel.text = [NSString stringWithFormat:@"%.01f", _eitherExpansionBlurStepper.value];
                
                _strokeEffectSwitch.on = [_glyphButton glyphLabelForState:UIControlStateNormal].showIndividualGlyphGlow;
                break;
                
            case DHSGlyphDemoComponentControlTypeShadow:
                _eitherExpansionBlurStepper.value = [_glyphButton glyphLabelForState:UIControlStateNormal].shadowBlur;
                _eitherExpansionBlurAmtLabel.text = [NSString stringWithFormat:@"%.01f", _eitherExpansionBlurStepper.value];
        ...
        switch (_choiceComponentControl.selectedSegmentIndex) {
            case DHSGlyphDemoComponentControlTypeFill:
                _eitherExpansionBlurStepper.value = _glyphLabel.currentFont.glyphExpansionMultiplier;
                _eitherExpansionBlurAmtLabel.text = [NSString stringWithFormat:@"%.01f", _eitherExpansionBlurStepper.value];
            ...
            case DHSGlyphDemoComponentControlTypeStroke:
                _eitherExpansionBlurStepper.value = _glyphLabel.currentFont.scaleFactor.x;
                _eitherExpansionBlurAmtLabel.text = [NSString stringWithFormat:@"%.01f", _eitherExpansionBlurStepper.value];
...
- (IBAction)expansionBlurStepperTap:(id)sender {
    if ([self isCurrentChoiceButton]) {
        ...
        switch (_choiceComponentControl.selectedSegmentIndex) {
            case DHSGlyphDemoComponentControlTypeFill:
                [_glyphButton setFontGlyphExpansionMultiplier:_eitherExpansionBlurStepper.value];
                break;

            case DHSGlyphDemoComponentControlTypeStroke:
                [_glyphButton setFontScaleFactor:CGPointMake(_eitherExpansionBlurStepper.value, 1.0f)];
                break;
        ...
        switch (_choiceComponentControl.selectedSegmentIndex) {
            case DHSGlyphDemoComponentControlTypeFill:
                [_glyphLabel setFontGlyphExpansionMultiplier:_eitherExpansionBlurStepper.value];
                break;
                
            case DHSGlyphDemoComponentControlTypeStroke:
                [_glyphLabel setFontScaleFactor:CGPointMake(_eitherExpansionBlurStepper.value, 1.0f)];
                break;
...
- (IBAction)colorButtonTap:(UIButton *)sender {
    if ([self isCurrentChoiceButton]) {
        ...
        switch (_choiceComponentControl.selectedSegmentIndex) {
            case DHSGlyphDemoComponentControlTypeFill:
                [_glyphButton setTitleColor:sender.backgroundColor forState:UIControlStateNormal];
                break;
            ...
            case DHSGlyphDemoComponentControlTypeShadow:
                [_glyphButton setTitleShadowColor:sender.backgroundColor forState:UIControlStateNormal];
                break;
