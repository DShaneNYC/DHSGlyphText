...
- (id)init {
  if (self = [super init]) {
    ...
    _cacheLevel1Size = [self
            defaultNumLevel1Items];  // DHSObject2LevelCacheDefaultLevel1MaxItems;
    _cacheMaxItemsSize =
        [self defaultNumMaxItems];  // DHSObject2LevelCacheDefaultTotalMaxItems;
    _saved = NO;

    _cacheItems =
        [[NSMutableDictionary alloc] initWithCapacity:_cacheMaxItemsSize];
    _orderedKeyList =
        [[NSMutableArray alloc] initWithCapacity:_cacheMaxItemsSize];
  }
...
- (NSArray *)keyFilesInObjectCachePath {
  ...
  NSMutableArray *files =
      [NSMutableArray arrayWithArray:[[NSFileManager defaultManager]
...
- (BOOL)isCorrupt {
  ...
  [_orderedKeyList
      enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        BOOL currentIsNull =
            [[_cacheItems objectForKey:obj] isKindOfClass:[NSNull class]] ? YES
                                                                          : NO;
...
- (NSString *)description {
  ...
  return [NSString
      stringWithFormat:@"Corrupt?: %@\nTotal: %lu of %ld, L1: %ld keys and %ld "
                       @"items of %ld, L2:%ldkeys and %ld items of %ld",
                       [self isCorrupt] ? @"YES" : @"No",
                       (unsigned long)[_orderedKeyList count],
                       (long)_cacheMaxItemsSize, (long)[self countOfL1Keys],
                       (long)[self countOfL1Items], (long)_cacheLevel1Size,
                       (long)[self countOfL2Keys], (long)[self countOfL2Items],
                       (long)_cacheMaxItemsSize - (long)_cacheLevel1Size];
}
...
- (NSInteger)defaultNumMaxItems {
  // Subclasses must overload this in order to change the default total cache
  // size
...
- (void)removeFiles {
  [[NSFileManager defaultManager] removeItemAtPath:[self objectCachePath]
                                             error:NULL];
}
...
- (void)clean {
  ...
  NSArray *files = [self keyFilesInObjectCachePath];
  [files enumerateObjectsUsingBlock:^(NSString *key, NSUInteger idx,
                                      BOOL *stop) {
    NSError *error;
    if ([_cacheItems objectForKey:key] == nil ||
        [_orderedKeyList indexOfObject:key] == NSNotFound) {
      [[NSFileManager defaultManager] removeItemAtPath:[self pathWithkey:key]
                                                 error:&error];
      [_orderedKeyList removeObject:key];
...
- (BOOL)sendToLevel2Object:(id<NSCoding>)obj
                    forKey:(id<NSCoding, NSCopying>)key {
...
- (BOOL)save {
  ...
  @synchronized(self) {
    ...
    [_orderedKeyList enumerateObjectsUsingBlock:^(NSString *key, NSUInteger idx,
                                                  BOOL *stop) {
    ...
    NSDictionary *dict = [[NSDictionary alloc]
        initWithObjectsAndKeys:_orderedKeyList, @"_orderedKeyList",
                               [NSNumber numberWithLong:_cacheMaxItemsSize],
                               @"_cacheMaxItemsSize",
                               [NSNumber numberWithLong:_cacheLevel1Size],
                               @"_cacheLevel1Size", nil];
...
- (BOOL)load {
  ...
  @synchronized(self) {
    ...
    NSInteger cacheMaxItemsSize =
        [[dict objectForKey:@"_cacheMaxItemsSize"] intValue];
    NSInteger cacheLevel1Size =
        [[dict objectForKey:@"_cacheLevel1Size"] intValue];
    if (newKeyList == nil) return NO;

    _cacheItems = nil;
    _cacheItems =
        [[NSMutableDictionary alloc] initWithCapacity:cacheMaxItemsSize];
    _orderedKeyList = nil;
    ...
    [newKeyList enumerateObjectsUsingBlock:^(NSString *key, NSUInteger idx,
                                             BOOL *stop) {
...
- (void)setCacheLevel1Size:(NSInteger)newCacheSize {
  ...
  if (newCacheSize > _cacheMaxItemsSize) {
    ...
    // No need to get the extra items from the L2 area since they will be loaded
    // as needed
...
- (void)removeObjectForKey:(id<NSCoding, NSCopying>)key {
  @synchronized(self) {
    // NSLog(@"del %@: %@", NSStringFromClass([self class]), [self
    // description]);
...
- (id<NSCoding>)objectForKey:(id<NSCoding, NSCopying>)key {
  @synchronized(self) {
    // NSLog(@"get %@: %@", NSStringFromClass([self class]), [self
    // description]);
...
- (void)setObject:(id<NSCoding>)object forKey:(id<NSCoding, NSCopying>)key {
  @synchronized(self) {
    // NSLog(@"set %@: %@", NSStringFromClass([self class]), [self
    // description]);
...
forKeyedSubscript:(id<NSCoding, NSCopying>)key {
