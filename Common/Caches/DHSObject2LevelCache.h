...
/**
 * Called on a delegate object just before an item stored in a cache will be
 *removed
 *
 ...
 */
- (void)DHScache:(DHSObject2LevelCache *)cache
 willEvictObject:(id<NSCoding>)obj
          forKey:(id<NSCoding>)key;

@end

@interface DHSObject2LevelCache : NSObject

/// The delegate of the cache that will implement the \b
/// DHSObject2LevelCacheDelegate protocol
...
/// The total size of the cache including the L1 (in memory) and L2 (disk)
...
/**
 * The unique private name of the cache directory to be used for storing the L2
 *portion of the cache.
 * It is a single component and not a full path. The default is the class name
 *of the cache.
 *
...
/**
 * The default number of items to store in the L1 (in memory) portion of the
 *cache
 *
...
/**
 * Remove all items from the cache, in memory and on disk, including any
 *evidence of saved metadata.
...
/**
 * Make sure all items in the cache and on disk are accounted for in the cache
 *metadata.
 * Remove any keys or items that can not be reconciled.
 *
 */
- (void)
    clean;  // Delete disk objects that should not be there in the first place
...
/**
 * Delete some (or all) of the low priority items in the cache, in memory or on
 *disk.
 * This does not change the size of the cache.
 *
...
/// Method used for object subscripting syntactic sugar equivalent to \b
/// objectForKey:
