...
// DHS: The following were added to remove warnings
short getInt16WithByteIndex(CFDataRef data, CFIndex index);
long getInt32WithByteIndex(CFDataRef data, CFIndex index);
UInt8 getUInt8WithByteIndex(CFDataRef data, CFIndex index);
...
short getInt16WithByteIndex(CFDataRef data, CFIndex index) {
    ...
    return (short)NSSwapBigShortToHost(value);
}
...
long getInt32WithByteIndex(CFDataRef data, CFIndex index) {
    ...
    return (long)NSSwapBigLongToHost(value);
}

// DHS: helper function
UInt8 getUInt8WithByteIndex(CFDataRef data, CFIndex index) {
    UInt8 value = 0;
    CFDataGetBytes(data, CFRangeMake(index, 1), (UInt8 *)&value);
    return value;
}
...
struct CMap {
    ...
    CGFloat descenderRatio; // DHS added
};

//
// DHS: The following were added to remove warnings
//
CMap * CMapCreate(CFDataRef fontTable, UInt32 segmentOffset, int format, CGFloat descenderRatio);
void CMapRelease(CMap * cmap);
CGFloat CMGetDescenderRatioForFont(CGFontRef cgFont);
CMap * CMGetCMapForFont(CGFontRef cgFont);
size_t CMap4GetGlyphIndicesForUnichar(CMap * cmap, UniChar const * s, CGGlyph * result, size_t count);
size_t CMap12GetGlyphIndicesForUnichar(CMap * cmap, UniChar const * s, CGGlyph * result, size_t count);
...
CMap * CMapCreate(CFDataRef fontTable, UInt32 segmentOffset, int format, CGFloat descenderRatio) 
{
	...
	cmap->format = format;
    cmap->descenderRatio = descenderRatio;
	return cmap;
}
...
CGFloat CMGetDescenderRatioForFont(CGFontRef cgFont) 
{
    CGFloat descenderRatio = 0.0f;

    // DHS: Get hhea
    // This is better because it comes from the font designer rather than being calculated
    CFDataRef fontHhea = CGFontCopyTableForTag(cgFont, 'hhea');
    if (fontHhea) {
        short ascent = (short)getInt16WithByteIndex(fontHhea, 4);
        short descent = (short)getInt16WithByteIndex(fontHhea, 6);
       //  short lineGap = (short)getInt16WithByteIndex(fontHhea, 8);
        CFRelease(fontHhea);

        short height = ascent - descent; // descent is negative, so get abs
        if (height > 0) {
            descenderRatio = -(CGFloat)descent / (CGFloat)height;
            
            // debugLog(@"hhea: ascent = %d, descent = %d, lineGap = %d, descenderRatio = %g", ascent, descent, lineGap, descenderRatio);
    ...
    // DHS: Get head
    // Use this only if the other isn't available
    CFDataRef fontHead = CGFontCopyTableForTag(cgFont, 'head');
    if (fontHead) {
		short yMin = (short)getInt16WithByteIndex(fontHead, 38);
		short yMax = (short)getInt16WithByteIndex(fontHead, 42);
        CFRelease(fontHead);

        short height = yMax - yMin; // yMin is neg
        
        if (height > 0) {
            descenderRatio = fabsf((CGFloat)yMin / (CGFloat)height);
            
            // debugLog(@"head: yMin = %d, yMax = %d, height = %d", yMin, yMax, height);
	...
	if (fontTable != NULL) {
		...
		if (bestPlatformID == 0 || bestPlatformID == 3) {
			...
			if (format == 4 || format == 12) {
				cmap = CMapCreate(fontTable, segmentOffset, format, CMGetDescenderRatioForFont(cgFont));
...
// DHS
CGFloat
CMFontGetDescenderRatio(CGFontRef cgFont)
{
	CMap * cmap = CMGetCMapForFont(cgFont);
    if (cmap) return cmap->descenderRatio;
    else return 0.0f;
}
