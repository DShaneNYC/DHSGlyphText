 ...
 *
 * @discussion This class is responsible for holding the information about a
 *display font including its glyphs.
 *
 */
...
/// The scale of the glyphs' width (x) and height (y) with respect to the
/// default glyph sizes in the font file
...
/// The amount the glyphs should be shifted together or apart with respect to
/// the default spacing in the font file
///
 ...
 *
 * @return Returns the optical bounding box of the requested character if it
 *exists in the font
 */
- (CGSize)sizeForUnichar:(unichar)letter;

/**
 * Find the bounding box size of the space glyph in the font at the current font
 *size
 *
 * @return Returns the optical bounding box of the space character if it exists
 *in the font
 */
- (CGSize)spaceSize;
 ...
 *
 * @param glyphs A pointer to previously allocated memory that can hold all the
 *glyphs for the text provided
 * @param text A string of text to retrieve glyphs for. If a glyph does not
 *exist in the font, zero will be placed at that location
 *
...
/**
 * Determine if all the characters in the provided string can be rendered in the
 *font
 *
...
/**
 * Determine if another font's properties are the same values as this font's
 *properties by comparing each property individually
 *
 * @param font The other font to compare
 *
 * @return Returns YES if all the properties are the same value, NO if any are
 *not
 */
