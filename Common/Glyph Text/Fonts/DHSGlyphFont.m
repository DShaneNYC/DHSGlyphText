...
- (NSString *)description {
  return [NSString stringWithFormat:@"Name: %@, Size: %f, Scale: (%f, %f), "
                                    @"DescRatio: %f, GlyphSpaceMult: %f",
                                    _fontName, _fontSize, _scaleFactor.x,
                                    _scaleFactor.y, _descenderRatio,
                                    _glyphExpansionMultiplier];
}

- (void)processFontInfo {
  if (_cgFont) {
    CGFontRelease(_cgFont), _cgFont = NULL;
  }
  if (_ctFont) {
    CFRelease(_ctFont), _ctFont = NULL;
  }
  ...
  NSString *fontPath =
      [[NSBundle mainBundle] pathForResource:_fontName ofType:nil];
  ...
  if (fontPath) {
    ...
    // Core Text
    if (_cgFont)
      _ctFont = CTFontCreateWithGraphicsFont(_cgFont, _fontSize, NULL, 0);
  } else {
    // Core Graphics
...
- (void)setFontName:(NSString *)fontName {
  if (_fontName == fontName || [_fontName isEqualToString:fontName] ||
      fontName == nil)
    return;
  _fontName = fontName;
...
- (void)setGlyphExpansionMultiplier:(CGFloat)glyphExpansionMultiplier {
  if (_glyphExpansionMultiplier == glyphExpansionMultiplier ||
      glyphExpansionMultiplier < 0.0f)
    return;
...
- (NSString *)hashForText:(NSString *)text {
  ...
  return [NSString
      stringWithFormat:@"%lX",
                       (unsigned long)text.hash ^ (unsigned long)self.hash ^
                           (unsigned long)
                               [[NSValue valueWithCGSize:CGSizeZero] hash] ^
...
- (CGSize)sizeForUnichar:(unichar)letter {
  ...
  return CGSizeMake(
      rect.size.width * _glyphExpansionMultiplier * _scaleFactor.x,
      rect.size.height * _scaleFactor.y);
}
...
- (CGSize)sizeForText:(NSString *)text {
  ...
  CGRect *rects =
      (CGRect *)malloc(sizeof(CGRect) * text.length);  // rects[length];
  CGPoint *points =
      (CGPoint *)malloc(sizeof(CGPoint) * text.length);  // points[length];

  // CGRect fullBoundingRect = CTFontGetBoundingRectsForGlyphs(useFont,
  // kCTFontHorizontalOrientation, textToPrint, rects, length);
  CGRect fullBoundingRect =
      CTFontGetOpticalBoundsForGlyphs(_ctFont, glyphs, rects, text.length, 0);
  ...
  for (NSInteger i = 0; i < text.length; ++i) {
    ...
    xOffset +=
        (_glyphExpansionMultiplier * _scaleFactor.x * rects[i].size.width);
  }
  // Adjust width for last character
  xOffset -= (rects[text.length - 1].size.width -
              (_glyphExpansionMultiplier * _scaleFactor.x *
               rects[text.length - 1].size.width));
  fullBoundingRect =
      CGRectMake(fullBoundingRect.origin.x, fullBoundingRect.origin.y, xOffset,
                 fullBoundingRect.size.height * _scaleFactor.y);
  debugLog(@"Full Rect %@", [NSValue valueWithCGRect:fullBoundingRect]);
  free(rects);
  free(points);
  free(glyphs);
...
- (BOOL)getGlyphs:(CGGlyph *)glyphs forText:(NSString *)text {
  ...
  BOOL retval =
      CTFontGetGlyphsForCharacters(_ctFont, unichars, glyphs, text.length);
...
- (NSUInteger)hash {
  return [_fontName hash] ^ [@(_fontSize) hash] ^
         [@(_scaleFactor.x + _scaleFactor.y * 2000.0f) hash] ^
         [@(_descenderRatio) hash] ^ [@(_glyphExpansionMultiplier) hash];
}
