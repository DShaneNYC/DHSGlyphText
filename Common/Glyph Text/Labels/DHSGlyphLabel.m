...
- (NSDictionary *)layoutInfo {
  ...
  if ([self.typesetter isKindOfClass:[DHSGlyphTypesetter class]]) {
    // For subclasses that need to call super (but not this one since it has no
    // super)
...
- (void)setShouldCache:(BOOL)shouldCache {
  ...
  [_fonts enumerateKeysAndObjectsUsingBlock:^(NSString *key, DHSGlyphFont *obj,
                                              BOOL *stop) {
...
- (void)setGlyphRenderOrder:(DHSGlyphLabelGlyphRenderOrder)glyphRenderOrder {
  if (_glyphRenderOrder == glyphRenderOrder ||
      glyphRenderOrder >= DHSGlyphLabelGlyphRenderOrderCount)
    return;
...
- (DHSGlyphFont *)bestFontWithGlyphs:(BOOL)getGlyphs {
  ...
  [_fonts enumerateKeysAndObjectsUsingBlock:^(NSString *key, DHSGlyphFont *obj,
                                              BOOL *stop) {
    if ([key isEqualToString:kDHSGlyphDefaultKey] == NO &&
        [key isEqualToString:kDHSGlyphSystemKey] == NO) {
      if (getGlyphs) {
...
- (BOOL)setFontName:(NSString *)fontName forKey:(NSString *)key {
  ...
  if (font) {
    ...
    font = [[DHSGlyphFont alloc]
        initWithFontName:fontName
                 andSize:self.font.pointSize > 0.0f
                             ? self.font.pointSize
                             : DHSGLYPH_DEFAULT_FONT_SIZE];
...
- (void)processRenderOrder:(DHSGlyphLabelGlyphRenderOrder)renderOrder
  ...
  // Subclasses can extend DHSGlyphLabelGlyphRenderOrder to add new rendering
  // orders
  ...
  switch (renderOrder) {
    ...
    case DHSGlyphLabelGlyphRenderOrderOutsideIn:
      for (NSInteger i = 0; i < (length >> 1); ++i) {
        NSInteger index = i << 1;
        debugLog(@"OI index order: %ld=%ld, %ld=%ld", (long)index, (long)i,
                 (long)index + 1, (long)length - i - 1);
...
- (void)reorderRenderValuesForIndexes:(NSInteger *)indexes
  ...
  if (glyphs == NULL || points == NULL || sizes == NULL || rotations == NULL ||
      length == 0)
    return;

  // glyhps, points and rotations arrays must already be allocated with size
  // length
  ...
  if (_rotations[0] == DHSGLYPH_NO_ROTATION)
    rotations[0] = DHSGLYPH_NO_ROTATION;
}
...
- (void)drawGlyph:(CGGlyph)glyph
  ...
  if (_currentFont.descenderRatio == MAXFLOAT) {
    ...
    myDescender = _currentFont.descenderRatio *
                  (_currentFont.fontSize * _currentFont.scaleFactor.y);
  }
  CGFloat centerY = point.y + myDescender;

  // Handle rotation
  CGAffineTransform textTransform =
      CGAffineTransformTranslate(textMatrixSave, centerX, centerY);
...
- (void)drawGlyphs:(CGGlyph *)glyphs
  ...
  // e.g. If glyphs had individual fill colors, that would be decided here, but
  // set and rendered in
  ...
  if (rotations[0] == DHSGLYPH_NO_ROTATION) {
    ...
    // Rotation or other individual glyph effects require glyphs be rendered
    // individually
    for (NSInteger i = 0; i < length; ++i) {
...
- (void)drawLabelWithContext:(CGContextRef)context
               ...
               withRotations:(CGFloat *)rotations
                   andLength:(NSInteger)length {
  debugLog(@"Attempting to draw label text: %@ (%@)", self.text,
  ...
  CGAffineTransform textTransform = CGAffineTransformMakeScale(
      _currentFont.scaleFactor.x, -_currentFont.scaleFactor.y);
  CGContextSetTextMatrix(context, textTransform);
  ...
  if (self.glowColor && self.glowBlur > 0.0f) {
    ...
    CGContextSetShadowWithColor(context, CGSizeMake(0.0f, 0.0f), self.glowBlur,
                                self.glowColor.CGColor);
  ...
  if (self.strokeHasShadow == NO || useStrokeWidth == 0.0f ||
      CGSizeEqualToSize(self.shadowOffset, CGSizeZero) ||
      useShadowColor == NULL) {
    // Without stroke shadow

    if (useShadowColor &&
        CGSizeEqualToSize(self.shadowOffset, CGSizeZero) == NO) {
      if (self.shiftsToShadowOffset) {
        // Put the text over the shadow and move the whole thing with an
        // adjustment
        CGContextSetShadowWithColor(context, CGSizeZero, self.shadowBlur,
                                    useShadowColor);
      } else {
        CGContextSetShadowWithColor(context, self.shadowOffset, self.shadowBlur,
                                    useShadowColor);
      }
  ...
  if (useFillColor && useStrokeWidth > 0.0f && useStrokeColor) {
    // Stroke and Fill
    CGContextSetTextDrawingMode(context, kCGTextFillStroke);
  } else if (self.gradient || (useFillColor && (useStrokeWidth == 0.0f ||
                                                useStrokeColor == NULL))) {
    // Fill Only
    CGContextSetTextDrawingMode(context, kCGTextFill);
  } else {
  ...
  if (self.gradient) {
    ...
    CGContextScaleCTM(context, _currentFont.scaleFactor.x,
                      -_currentFont.scaleFactor.y);
    ...
    if (_radialGradient) {
      CGPoint start =
          CGPointMake(CGRectGetMidX(rect) / _currentFont.scaleFactor.x,
                      -CGRectGetMidY(rect) / _currentFont.scaleFactor.y);
      CGPoint end = start;
      CGContextDrawRadialGradient(context, self.gradient, start, 0.0f, end,
                                  CGRectGetHeight(rect) /
                                      _currentFont.scaleFactor.x /
                                      _currentFont.scaleFactor.y,
                                  kCGGradientDrawsAfterEndLocation);
    } else {
      CGPoint start = rect.origin;
      CGPoint end = CGPointMake(
          rect.origin.x / _currentFont.scaleFactor.x,
...
- (void)layoutLabelInRect:(CGRect)rect {
  debugLog(@"Attempting to layout label text: %@ (%@)", self.text,
           _currentFont.fontName);
  ...
  // Make space for rotations and set first value with NO ROTATION (for entire
  // string)
...
- (NSUInteger)hash {
  ...
  return
      [@(self.bounds.size.width * 1000.0f + self.bounds.size.height * 4000.0f)
              hash] ^
      self.class.hash ^ self.text.hash ^ self.textColor.hash ^
      _currentFont.hash ^ _typesetter.class.hash ^ _typesetter.layoutInfo.hash ^
      (_strokeColor.hash << 1) ^ [@(_strokeWidth * 10000.0f) hash] ^
      (self.shadowColor.hash << 2) ^ (_strokeHasShadow ? 1 : 0) ^
      [@(_shadowBlur * 100.0f) hash] ^
      [@(self.shadowOffset.width + self.shadowOffset.height * 2000.0f) hash] ^
      (_glowColor.hash << 3) ^ [@(_glowBlur * 1000.0f) hash] ^
      (_gradient ? 1 : 0) << 3 ^ [@(_radialGradient) hash] ^
      (_showIndividualGlyphStroke ? 1 : 0) << 1 ^
      (_showIndividualGlyphGlow ? 1 : 0) << 2 ^ _glyphRenderOrder << 4;
}
