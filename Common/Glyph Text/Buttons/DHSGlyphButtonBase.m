...
@implementation DHSGlyphButtonBase
...
- (DHSGlyphLabelBase *)setupLabelForState:(UIControlState)state {
    ...
    return [[DHSGlyphLabelBase alloc] initWithFrame:self.frame];
}

- (void)setupStates {
	_needsUpdate = YES;
    
	DHSGlyphLabelBase *normal = [self setupLabelForState:UIControlStateNormal];
	DHSGlyphLabelBase *highlighted = [self setupLabelForState:UIControlStateHighlighted];
	DHSGlyphLabelBase *disabled = [self setupLabelForState:UIControlStateDisabled];
	DHSGlyphLabelBase *selected = [self setupLabelForState:UIControlStateSelected];
	...
	for (NSNumber *key in _states) {
		...
		DHSGlyphLabelBase *label = [_states objectForKey:key];
...
- (void)setTitle:(NSString *)title {
	...
	for (NSNumber *state in _states) {
		DHSGlyphLabelBase *label = [_states objectForKey:state];
		[label setText:title];
	}
...
- (void)setTitle:(NSString *)title forState:(UIControlState)state {
	...
	DHSGlyphLabelBase *label = [_states objectForKey:[NSNumber numberWithInt:state]];
	[label setText:title];
	[self setImage:[label getImage] forState:state];
	...
	if (state == UIControlStateNormal) {
		...
		if (label.text == nil) {
			DHSGlyphLabelBase *normalLabel = [_states objectForKey:[NSNumber numberWithInt:UIControlStateNormal]];
...
- (BOOL)shouldCache {
    DHSGlyphLabelBase *label = [self glyphLabelForState:UIControlStateNormal];
