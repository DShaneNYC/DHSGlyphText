...
- (void)setupStates {
  ...
  DHSGlyphLabel *highlighted =
      [self setupLabelForState:UIControlStateHighlighted];
  ...
  _states = [NSDictionary
      dictionaryWithObjectsAndKeys:
          ...
          selected, [NSNumber numberWithInt:UIControlStateSelected], nil];
  ...
  for (NSNumber *key in _states) {
    ...
    [self setTitleColor:[self titleColorForState:[key intValue]]
               forState:[key intValue]];
    [self setTitleShadowColor:[self titleShadowColorForState:[key intValue]]
                     forState:[key intValue]];
...
- (NSString *)titleForState:(UIControlState)state {
    NSString *title = [[_states objectForKey:[NSNumber numberWithInt:state]]
text];

    if ([title length] > 0) return title;
    else return [[_states objectForKey:[NSNumber
numberWithInt:UIControlStateNormal]] text];
}
*/
...
- (void)setTitle:(NSString *)title forState:(UIControlState)state {
  ...
  if (state == UIControlStateNormal) {
    ...
    if (label.text == nil) {
      DHSGlyphLabel *normalLabel =
          [_states objectForKey:[NSNumber numberWithInt:UIControlStateNormal]];
