...
- (void)layoutGlyphs:(CGGlyph *)glyphs
  ...
  for (NSInteger i = 0; i < length; ++i) {
    ...
    if (horizontalMin <= horizontalMax)
      adjustX = randomFloat(horizontalMin, horizontalMax);
    if (verticalMin <= verticalMax)
      adjustY = randomFloat(verticalMin, verticalMax);
    if (doRotation && rotationMin <= rotationMax)
      adjustRotation = randomFloat(rotationMin, rotationMax);

    points[i] = CGPointMake(points[i].x * (1.0f + adjustX),
                            points[i].y * (1.0f + adjustY));
