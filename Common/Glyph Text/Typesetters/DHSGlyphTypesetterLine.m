...
+ (CGSize)sizeNoSpaceForText:(NSString *)text
  ...
  debugLog(@"NoS Label Text = %@ (length = %lu)", text,
  ...
  NSMutableArray *charSizes =
      [[NSMutableArray alloc] initWithCapacity:text.length];
  ...
  do {
    ...
    [lastList
        addObject:[NSNumber numberWithLong:lineFirstGlyph + glyphCount + 1]];
  ...
  CGFloat lineLayoutStyleMultiplier = [layoutInfo[
  ...
  CGFloat useHeight =
      numLines == 1 ? spaceHeight : (numLines - 1) * lineHeight + spaceHeight;
...
+ (CGSize)sizeForText:(NSString *)text
  ...
  NSArray *words = [[text
      stringByReplacingOccurrencesOfString:
          @"\n" withString:[NSString
                               stringWithFormat:
                                   @" %@ ", DHSGLYPHTYPESETTER_LINE_DELIMITER]]
  ...
  NSMutableArray *wordSizes =
      [[NSMutableArray alloc] initWithCapacity:[words count]];
  NSInteger i = 0;
  ...
  do {
    ...
    while (lastWord < [words count]) {
      ...
      // New line
      if ([[words objectAtIndex:lastWord]
              isEqualToString:DHSGLYPHTYPESETTER_LINE_DELIMITER]) {
        maxLineWidth =
            MAX(maxLineWidth,
                width - [[wordSizes objectAtIndex:lastWord] floatValue]);
        glyphCount -= [[words objectAtIndex:lastWord] length];
...
+ (NSDictionary *)defaultLayoutInfo {
  NSMutableDictionary *info =
      [NSMutableDictionary dictionaryWithDictionary:super.defaultLayoutInfo];
  ...
  [info addEntriesFromDictionary:@{
    ...
    @(DHSGlyphTypesetterLineLayoutInfoLineLayoutStyle) :
        @(DHSGlyphTypesetterLineLayoutStyleCenter),
...
- (void)layoutGlyphs:(CGGlyph *)glyphs
  ...
  debugLog(@"Attempting to layout glyphs text: %@ (%@)", self.text,
           font.fontName);
  ...
  if (myGlyphBodyRatio == MAXFLOAT)
    myGlyphBodyRatio =
        (CTFontGetDescent(font.ctFont) / (font.fontSize * font.scaleFactor.y));
  myGlyphBodyRatio = 1.0f - myGlyphBodyRatio;

  // CGRect fullBoundingRect =
  // CTFontGetBoundingRectsForGlyphs(_currentFont.ctFont,
  // kCTFontHorizontalOrientation, glyphs, rects, length);
  CGRect fullBoundingRect =
      CTFontGetOpticalBoundsForGlyphs(font.ctFont, glyphs, rects, length, 0);
  ...
  xOffset += (rects[length - 1].size.width -
              (font.glyphExpansionMultiplier * rects[length - 1].size.width));
  fullBoundingRect =
      CGRectMake(fullBoundingRect.origin.x, fullBoundingRect.origin.y, xOffset,
                 fullBoundingRect.size.height);
  // debugLog(@"Full Rect %@", [NSValue valueWithCGRect:fullBoundingRect]);
  free(rects);
  ...
  UITextAlignment textAlignment =
      [layoutInfo[@(DHSGlyphTypesetterLineLayoutInfoTextAlignment)] intValue];
  CGFloat strokeWidth =
      [layoutInfo[@(DHSGlyphTypesetterLineLayoutInfoStrokeWidth)] floatValue];
  CGSize shadowOffset =
      [layoutInfo[@(DHSGlyphTypesetterLineLayoutInfoShadowOffset)] CGSizeValue];
  ...
  if (textAlignment == NSTextAlignmentLeft) {
    ...
    adjustment.x =
        (rect.origin.x + (rect.size.width / myScaleFactor.x) - textEnd) / 2.0f;
  } else if (textAlignment == NSTextAlignmentRight) {
    adjustment.x = rect.origin.x + (rect.size.width / myScaleFactor.x) -
                   textEnd - strokeWidth;
  ...
  if ([layoutInfo[
          @(DHSGlyphTypesetterLineLayoutInfoShiftsToShadowOffset)] boolValue]) {
  ...
  for (NSInteger i = 0; i < length; ++i) {
    points[i] =
        CGPointMake(points[i].x + adjustment.x,
                    -(adjustment.y +
                      font.fontSize * font.scaleFactor.y * myGlyphBodyRatio));
...
- (BOOL)layoutInRect:(CGRect)rect {
  ...
  // Layout each line
  NSInteger numLines = [firstList count];
  NSInteger maxNumberOfLines = [layoutInfo[
      @(DHSGlyphTypesetterLineLayoutInfoMaxNumberOfLines)] intValue];
  ...
  switch (lineLayoutStyle) {
    ...
    case DHSGlyphTypesetterLineLayoutStyleCenter:
      lineHeight =
          lineLayoutStyleMultiplier * font.fontSize * font.scaleFactor.y;
      yOffset = (rect.size.height - (((CGFloat)numLines - 1.0f) * lineHeight +
                                     font.fontSize * font.scaleFactor.y)) /
                2.0f;
      yIncrement = 0.0f;
      break;
    ...
    case DHSGlyphTypesetterLineLayoutStyleSpread:
      if (numLines == 1) {
        yOffset = (rect.size.height -
                   ((CGFloat)numLines * font.fontSize * font.scaleFactor.y)) /
                  2.0f;
        yIncrement = 0.0f;
      } else {
        yOffset = 0.0f;
        yIncrement = ((rect.size.height - font.fontSize * font.scaleFactor.y) /
  ...
  debugLog(@"Text Length: %lu, Last Index: %d", (unsigned long)text.length,
           [[lastList lastObject] intValue]);
  debugLog(@"numPoints = %lu * %lu (%@)", sizeof(CGPoint),
  ...
  while (i < numLines) {
    ...
    debugLog(@"Line %ld from %ld to %ld (%@)", (long)i, (long)firstGlyph,
             (long)lastGlyph - 1,
             [text substringWithRange:NSMakeRange(firstGlyph, length)]);

    // Make the Rect
    lineY = i * lineHeight;
    CGRect lineRect = CGRectMake(lineX, lineY + yOffset, lineWidth,
                                 font.fontSize * font.scaleFactor.y);

    // Layout the text
    debugLog(@"Location %p = %p + %lu", points + firstGlyph, points,
             firstGlyph * sizeof(CGPoint));
